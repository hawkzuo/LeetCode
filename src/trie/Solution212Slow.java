package trie;
import java.util.*;
//Plus linkedlist.iterator tests;
final class Point2D{
	final int x;
	final int y;
...
public class Solution212Slow {

	Map<Character,Map<Point2D,Integer>> tb;
	Point2D[][] innerPts;
    ...
    public List<String> findWords(char[][] board, String[] words) {
    	...
    	tb = new HashMap<Character,Map<Point2D,Integer>> ();
    	
    	Map<String,Integer> lookuptable = new HashMap<String,Integer>();
    	int m = board.length;
    	int n = board[0].length;
    	innerPts = new Point2D[m][n];
    	...
    	for(int i=0;i<m;i++){
    		for(int j=0;j<n;j++){
    			...
    			if(tb.containsKey(board[i][j])){
    				tb.get(board[i][j]).put(innerPts[i][j], 1);
    			}else{
    				Map<Point2D,Integer> nptm = new HashMap<Point2D,Integer>();
    				Point2D npt = innerPts[i][j];
    				nptm.put(npt, 1);
    				tb.put(board[i][j], nptm);
    			}
    	...
    	for(String str: words){
    		...
    		if(tb.containsKey(str.charAt(0))){
    			Map<Point2D,Integer> pts = tb.get(str.charAt(0));
    			Map<Point2D,Integer> ptList = new HashMap<Point2D,Integer>();
    			boolean found = false;
    			for(Point2D startpoint : pts.keySet()){
    	    		ptList.put(startpoint, 1);
    				if(dfs(ptList,str,1,startpoint,board)){
    	    			found = true;
    	    			break;
    	    		}
    			}
    ...
    private boolean dfs(Map<Point2D,Integer> pList,String input,int curstep, Point2D curpoint,char[][] board){
    	 	
    	int m = board.length;
    	int n = board[0].length; 
    	if(curstep == input.length()){
    		return true;
    	}
    	
    	
    	List<Point2D> nbs = getNbs(curpoint,m,n,board,input,curstep,pList);
    	
    	if(nbs.size() == 0){
    		//BT
    		pList.remove(curpoint);
    		return false;
    	}
    	boolean result = false;
    	for( Point2D newpt: nbs){
    		pList.put(newpt, 1);
    		result = dfs(pList,input,curstep+1,newpt,board);
    		if(result == true){
    			return result;
    		}    		
    	}
    ...
    private  List<Point2D> getNbs(Point2D ipt, int xlim, int ylim,char[][] board,String input,int curstep,Map<Point2D,Integer> pList){
    	List<Point2D> res = new ArrayList<Point2D>();
    	if(ipt.x >0){
    		if(board[ipt.x-1][ipt.y] == input.charAt(curstep)){   			
    			Point2D candi = innerPts[ipt.x-1][ipt.y];
    			
    			if(!pList.containsKey(candi)){
    				res.add(candi);
    			}
    		}
    	}
    	...
    	if(ipt.y >0){
    		if(board[ipt.x][ipt.y-1] == input.charAt(curstep)){
    			Point2D candi = innerPts[ipt.x][ipt.y-1];
    			if(!pList.containsKey(candi)){
    				res.add(candi);
    			}
    		}
	...
	public static void main(String[] args) {
		...
		Solution212Slow s = new Solution212Slow();
		char[][] board1 = {{'o','a','t','n'},{'a','t','a','e'},{'i','h','k','r'},{'i','f','l','v'}};
		String [] strs1 = {"oaaneateihkrvlfi","pea","eat","rain"};
		
		char[][] board = new char[11][4];
		...
		String []  sboard= {"aaaa","aaaa","aaaa","aaaa","bcde","fghi","jklm","nopq","rstu","vwxy","zzzz"};	
		for(int i=0;i<sboard.length;i++){
			board[i] = sboard[i].toCharArray();
		}
		for(String str: s.findWords(board, strs)){
